...
public class PanelUsuarios extends JPanel {
	...
	public PanelUsuarios() {
		...
		lblSuperiorTema.setFont(FONT_PANEL_TITLE);
		lblSuperiorTema.setForeground(COLOR_TEXT_NEGRO);
		lblSuperiorTema.setHorizontalAlignment(SwingConstants.CENTER);
		gbc.gridx = 0;
		gbc.gridy = 0;
		gbc.gridwidth = 4;
		...
		panelContenido.add(lblSuperiorTema, gbc);
		gbc.gridwidth = 1;
		...
		panelContenido.add(lblNombreUsuario, gbc);

		gbc.gridx = 1;
		gbc.fill = GridBagConstraints.HORIZONTAL;
		textNombre = new JTextField(20);
		styleTextField(textNombre, true, "Nombre del usuario");
		...
		styleLabel(lblApellidoUsuario);
		panelContenido.add(lblApellidoUsuario, gbc);

		gbc.gridx = 1;
		gbc.fill = GridBagConstraints.HORIZONTAL;
		textApellido = new JTextField(20);
		styleTextField(textApellido, true, "Apellido del usuario");
		panelContenido.add(textApellido, gbc);
		gbc.fill = GridBagConstraints.NONE;
		...
		styleTextField(textUsuario, true, "Nombre de acceso");
		panelContenido.add(textUsuario, gbc);
		gbc.fill = GridBagConstraints.NONE;

		gbc.gridy = 5;
		gbc.gridx = 0;
		JLabel lblPasswordUsuario = new JLabel("Contraseña:");
		styleLabel(lblPasswordUsuario);
		panelContenido.add(lblPasswordUsuario, gbc);

		gbc.gridx = 1;
		gbc.fill = GridBagConstraints.HORIZONTAL;
		textPassword = new JPasswordField(20);
		stylePasswordField(textPassword, true, "Ingrese contraseña");
		panelContenido.add(textPassword, gbc);
		gbc.fill = GridBagConstraints.NONE;
		...
		styleButton(btnAgregarUsuario);
		btnAgregarUsuario.setIcon(IconFontSwing.buildIcon(FontAwesomeSolid.USER_PLUS, 15, COLOR_TEXT_NEGRO));
		gbcBotones.gridx = 0; gbcBotones.gridy = 0; panelBotones.add(btnAgregarUsuario, gbcBotones);

		btnBuscarUsuario = new JButton("Buscar");
		styleButton(btnBuscarUsuario);
		btnBuscarUsuario.setIcon(IconFontSwing.buildIcon(FontAwesomeSolid.SEARCH, 15, COLOR_TEXT_NEGRO));
		gbcBotones.gridx = 1; gbcBotones.gridy = 0; panelBotones.add(btnBuscarUsuario, gbcBotones);

		btnEditarUsuario = new JButton("Editar");
		styleButton(btnEditarUsuario);
		btnEditarUsuario.setIcon(IconFontSwing.buildIcon(FontAwesomeSolid.USER_EDIT, 15, COLOR_TEXT_NEGRO));
		gbcBotones.gridx = 0; gbcBotones.gridy = 1; panelBotones.add(btnEditarUsuario, gbcBotones);

		btnEliminar = new JButton("Eliminar");
		styleButton(btnEliminar);
		btnEliminar.setIcon(IconFontSwing.buildIcon(FontAwesomeSolid.USER_MINUS, 15, COLOR_TEXT_NEGRO));
		gbcBotones.gridx = 1; gbcBotones.gridy = 1; panelBotones.add(btnEliminar, gbcBotones);
		...
		tablaUsuarios.setModel(new DefaultTableModel(
				new Object[][] {},
				new String[] {
						"ID", "NOMBRE", "APELLIDO", "USUARIO" // Contraseña no se muestra en la tabla
				}
		));
	...
	private void stylePasswordField(JPasswordField passwordField, boolean isEditable, String placeholder) {
		passwordField.setFont(FONT_INPUT);
		passwordField.setMargin(new Insets(2, 6, 2, 6));
		Border lineBorder = new MatteBorder(0, 0, 1, 0, COLOR_DIVIDER_POR_DEFECTO);
		passwordField.setForeground(COLOR_TEXT_NEGRO);

		if (isEditable) {
			if (placeholder != null && !placeholder.isEmpty()) {
				new TextPlaceholder(placeholder, passwordField, COLOR_PLACEHOLDER, FONT_INPUT.deriveFont(Font.ITALIC));
			}
		} else {
			passwordField.setEditable(false);
		}
		passwordField.setBorder(new CompoundBorder(lineBorder, new EmptyBorder(5, 8, 5, 8)));
		passwordField.addFocusListener(new FocusAdapter() {
			Border originalBorder = passwordField.getBorder();
			Border focusedBorder = new CompoundBorder(new MatteBorder(0,0,2,0, COLOR_TEXT_NEGRO), new EmptyBorder(5,8,4,8));
			@Override public void focusGained(FocusEvent e) {
				// L&F should handle focus indication
			}
	...
	private static class TextPlaceholder extends FocusAdapter implements Border {
		...
		private char defaultEchoChar; // For JPasswordField
		...
		public TextPlaceholder(String placeholder, JTextComponent textComponent, Color placeholderColor, Font placeholderFont) {
			...
			if (textComponent instanceof JPasswordField) {
				this.defaultEchoChar = ((JPasswordField) textComponent).getEchoChar();
			}
		...
		public void focusLost(FocusEvent e) {
			String currentText = "";
			if (textComponent instanceof JPasswordField) {
				currentText = new String(((JPasswordField)textComponent).getPassword());
			} else {
				currentText = textComponent.getText();
			}

			if (currentText.trim().isEmpty()) {
				if(!showingPlaceholder) {
					// No need to save original if they are constants now
				}
				textComponent.setText(placeholder);
				textComponent.setForeground(placeholderColor);
				textComponent.setFont(placeholderFont);
				if (textComponent instanceof JPasswordField) {
					((JPasswordField) textComponent).setEchoChar((char) 0); // Show placeholder text
				}
				showingPlaceholder = true;
			} else {
				textComponent.setForeground(originalForeground);
				textComponent.setFont(originalFont);
				if (textComponent instanceof JPasswordField && !showingPlaceholder) { // Ensure echo char is set if it had actual text
					((JPasswordField) textComponent).setEchoChar(defaultEchoChar);
				}
				showingPlaceholder = false;
			}
